...
//!
//! Extends the deferred rendering demo to support multiple mesh instances:
//! - Loads a single GLTF mesh from disk (assets/duck.glb)
//! - Creates three instances of the same mesh with different transforms
...
//! - Auto-scales and centers the mesh, then positions instances in the scene
...
struct MeshTransform {
    ...
    center: Vector3<f32>,
    /// Offset for positioning the mesh in the scene.
...
pub struct DeferredRenderer {
    // Multiple mesh handles (all pointing to the same duck mesh)
    mesh_handles: Vec<MeshHandle>,
    // Per-mesh transform data
    mesh_transforms: Vec<MeshTransform>,

    // Geometry pass resources
    geometry_uniform_buffers: Vec<wgpu::Buffer>,
    geometry_bind_group_layout: wgpu::BindGroupLayout,
    geometry_bind_groups: Vec<wgpu::BindGroup>,
...
impl AppRenderer for DeferredRenderer {
    async fn init(mut context: RenderContext<'_>) -> Self {
        ...
        // Load the duck mesh three times (same mesh, different transforms)
        ...
        // Create three instances of the same mesh with different position offsets
        let position_offsets = [
            Vector3::new(-1.5, 0.0, 0.0), // Left
            Vector3::new(0.0, 0.0, 0.0),  // Center
            Vector3::new(1.5, 0.0, 0.0),  // Right
        ];
        ...
        for i in 0..3 {
            // All three instances use the same mesh handle
            mesh_handles.push(mesh_handle);

            // Each instance has the same scale and center (from the mesh asset)
            // but a different position offset
            let transform = MeshTransform {
                scale: mesh_asset.scale,
                center: mesh_asset.center,
        ...
        // Create geometry bind group layout (shared for all meshes)
        ...
        for i in 0..3 {
            ...
            let geometry_bind_group = create_uniform_bind_group(
                device,
                Some(&format!("Geometry Bind Group {}", i)),
                &geometry_bind_group_layout,
                &geometry_uniform_buffer,
            );

            geometry_uniform_buffers.push(geometry_uniform_buffer);
            geometry_bind_groups.push(geometry_bind_group);
        }
    ...
    fn render(&mut self, mut context: RenderContext<'_>) {
        ...
        // Pre-fetch all mesh resources to avoid borrow conflicts during rendering
        let mesh_resources: Vec<_> = self
            .mesh_handles
            .iter()
            .map(|&handle| {
                context
                    ...
                    .mesh_cache
                    .get_both(handle)
                    .map(|(_, resource)| (handle, resource.clone()))
                    .expect("Failed to get mesh data")
            })
            .collect();
        ...
        // =====================================================================
        {
            ...
            // Draw each mesh with its own transform and uniform buffer
            for (i, &(_mesh_handle, ref mesh_resource)) in mesh_resources.iter().enumerate() {
                let transform = &self.mesh_transforms[i];
                let num_indices = mesh_resource.num_indices;

                // Create model matrix: center, scale, position, rotate
                // All meshes rotate around global origin
                let position_translation = Matrix4::from_translation(transform.position_offset);
                let center_translation = Matrix4::from_translation(-transform.center);
                let scale = Matrix4::from_scale(transform.scale);
                ...
                // 1. Move to local center (undo original center offset)
                // 2. Scale
                // 3. Rotate
                // 4. Move to final position
                let model = position_translation * rotation * scale * center_translation;
                // Update geometry uniform buffer for this mesh
                let geometry_uniform = GeometryUniform::new(&camera, model, aspect);
                queue.write_buffer(
                    &self.geometry_uniform_buffers[i],
                    0,
                    bytemuck::cast_slice(&[geometry_uniform]),
                );

                // Set bind group for this mesh's uniform buffer
                geometry_pass.set_bind_group(0, &self.geometry_bind_groups[i], &[]);
                ...
                geometry_pass.draw_indexed(0..num_indices, 0, 0..1);
            }
        }
